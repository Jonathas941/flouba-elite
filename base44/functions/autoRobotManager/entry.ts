...
Deno.serve(async (req) => {
  try {
    ...
    // Get BotSettings (service role — no user in scheduled context)
    const settings = await base44.asServiceRole.entities.BotSettings.list();
    if (!settings?.length) return Response.json({ ok: true, message: "No BotSettings found" });
    ...
    if (config.auto_stop_enabled !== false && robotRunning) {
      ...
      if (dailyPnL >= profitTarget) {
        ...
        actions.push(`AUTO-STOP: Daily profit target $${profitTarget} reached (P&L: +$${dailyPnL.toFixed(2)})`);
      } else if (dailyPnL <= -lossLimit) {
        await fetch(`${BASE}/robot/stop`, { method: "POST", headers: authHeaders, body: "{}" });
        actions.push(`AUTO-STOP: Daily loss limit $${lossLimit} reached (P&L: -$${Math.abs(dailyPnL).toFixed(2)})`);
      }
    }
    ...
    if (config.auto_start_enabled && !robotRunning) {
      ...
      // 5-minute window (polling runs every 5 min)
      if (diff <= 5) {
        const startPayload = {
          strategy: "auto",
