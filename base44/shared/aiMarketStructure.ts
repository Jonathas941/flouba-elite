// ── AI Market Structure Scanner (OpenAI-powered SMC analysis) ───────────────
// Uses the shared OpenAI client to analyze live market data for SMC concepts:
// BOS, CHOCH, liquidity sweeps, FVG, order blocks, and market bias.
//
// Flow:
//   1. Shared OpenAI client (direct API → InvokeLLM with gpt_5_mini fallback)
//   2. Final fallback: EA-provided boolean flags
...
export async function aiMarketStructureScan(ind, regime, regimeDir, cfg, base44Client) {
  ...
  // ── 1. OpenAI (via shared client — direct API or gpt_5_mini fallback) ──
  const res = await callOpenAI({
    systemPrompt: "You are an expert Smart Money Concepts (SMC) analyst. Respond only with valid JSON matching the requested schema.",
    userPrompt: SMC_PROMPT(marketContext),
    schema: SMC_SCHEMA,
    temperature: 0.3,
    base44Client,
  });
  ...
  if (res) {
    return { ...parseResult(res), source: res._source || "openai" };
  }
