...
export default function ConnectMT5() {
  ...
  const handleTest = async () => {
    ...
    setStatus("testing");
    // Simulate a test ping — real MT5 bridge will replace this
  ...
  const handleConnect = async () => {
    if (!isFormValid) {
      toast({ title: "Fill in all fields to connect.", variant: "destructive" });
      return;
    }
        ...
        <GlassCard className="p-0 overflow-visible" style={{ zIndex: 50, position: "relative" }}>
          ...
          <div className="relative px-4 pb-4">
            <button
              ...
              className="w-full flex items-center justify-between h-11 px-3 glass rounded-xl text-sm font-semibold"
            >
              <span className={broker ? "text-white" : "text-muted-foreground/50"}>
                {broker || "Select your broker"}
              </span>
              <ChevronDown className={`w-4 h-4 text-muted-foreground transition-transform ${brokerOpen ? "rotate-180" : ""}`} />
            </button>
            {brokerOpen && (
              <motion.div
                initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }}
                className="absolute z-50 left-4 right-4 top-full mt-1 rounded-xl overflow-hidden"
                style={{ background: "rgba(18,18,18,0.98)", border: "1px solid rgba(239,68,68,0.15)" }}
              >
                {BROKERS.map((b) => (
                  <button
                    key={b}
                    onClick={() => { setBroker(b); setBrokerOpen(false); }}
                    className={`w-full text-left px-4 py-3 text-sm transition-colors hover:bg-red-500/10 border-b border-white/5 last:border-0 ${broker === b ? "text-red-400 font-bold" : "text-white/80"}`}
                  >
                    {b}
                  </button>
                ))}
        ...
        {status !== "success" && (
          <>
            ...
            <Button
              ...
              disabled={status === "testing" || status === "connecting" || !isFormValid}
            >
