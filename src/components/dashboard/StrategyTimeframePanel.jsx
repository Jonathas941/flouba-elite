...
function Chip({ label, value, tone = "neutral" }) {
  const tones = {
    ...
    cool: "text-[#00FF41] bg-[#00FF41]/10 border-[#00FF41]/25",
      ...
      <GlassCard className="p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-xl bg-[#00FF41]/15 flex items-center justify-center">
        ...
        <div className="flex items-center justify-between py-1.5 border-t border-white/5">
          ...
          <span className="text-[11px] font-heading font-bold text-[#00FF41]">{fmtTime(lastClose)}</span>
        </div>
        <div className="flex items-center justify-between py-1.5 border-t border-white/5">
          <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-heading font-bold text-orange-400">
            {secsToNext != null ? `${Math.floor(secsToNext / 60)}:${String(secsToNext % 60).padStart(2, "0")}` : "--"}
          </span>
        </div>

        {tf.note && (
          <div className="flex items-start gap-2 rounded-xl px-3 py-2" style={{ background: "rgba(0,255,65,0.05)", border: "1px solid rgba(0,255,65,0.15)" }}>
            <Activity className="w-3.5 h-3.5 text-[#00FF41] shrink-0 mt-0.5" />
