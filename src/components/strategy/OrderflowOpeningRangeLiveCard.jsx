...
import {
  ...
  Layers, Gauge, Lock, CheckCircle2, XCircle, Loader2, Clock, Crosshair, Box, Waves,
} from "lucide-react";

const STRATEGY_NAME = "Orderflow Opening Range Breakout";
...
function Chip({ label, value, tone = "neutral" }) {
  const tones = {
    ...
    cool: "text-fuchsia-300 bg-fuchsia-500/10 border-fuchsia-500/25",
  };
  return (
...
function StatusRow({ icon: Icon, label, value, tone }) {
  const toneClass = tone === "up" ? "text-[#00ff9d]" : tone === "down" ? "text-[#ff4d4d]" : tone === "warn" ? "text-amber-400" : tone === "cool" ? "text-fuchsia-300" : "text-white/70";
  return (
...
export default function OrderflowOpeningRangeLiveCard() {
  ...
  // Opening range high/low — prefer backend-computed fields, else fall back
  const orHigh = ind?.openingrange_high ?? ind?.ofor_high ?? null;
  const orLow = ind?.openingrange_low ?? ind?.ofor_low ?? null;
  const orMid = orHigh != null && orLow != null ? (orHigh + orLow) / 2 : null;

  // Phase determination (all times ET)
  const RANGE_START = 9 * 60 + 30;   // 09:30
  const RANGE_END = 10 * 60;          // 10:00
  ...
  const dailyLossHit = dailyLossPct <= -(s.ofor_max_daily_loss_pct ?? 2);
  const equityStopLevel = balance > 0 ? balance * (1 - (s.ofor_equity_stop_pct ?? 3) / 100) : null;
  ...
  const maxConsec = s.ofor_max_consecutive_losses ?? 2;
  const cooldownH = s.ofor_cooldown_hours ?? 8;
  ...
  const spreadOk = spread == null || spread <= (s.ofor_max_spread_points ?? 30);
  const rangeOk = orHigh != null && orLow != null;
  const rangeSize = rangeOk ? orHigh - orLow : null;
  const minRange = s.ofor_min_range_points ?? 20;
  const maxRange = s.ofor_max_range_points ?? 400;
  const rangeSizeOk = rangeSize != null ? rangeSize >= minRange && rangeSize <= maxRange : false;
  const atrOk = !(s.ofor_use_atr_filter !== false) || (atr != null && atr >= (s.ofor_min_atr ?? 0));
  const maxTrades = s.ofor_max_trades_per_day ?? 2;
  const capHit = tradesTakenToday >= maxTrades;

  // breakout status (price-based proxy; final confirmation requires closed M5 candle + order flow)
  let breakout = "Waiting";
  if (rangeOk && price != null) {
    if (price > orHigh) breakout = "Buy pending close";
    else if (price < orLow) breakout = "Sell pending close";
  }

  // order-flow proxy (volume expansion when available from scanner)
  const volExp = ind?.volume ?? ind?.vol ?? null;
  const volAvg = ind?.volume_avg ?? ind?.vol_avg ?? null;
  const volMult = s.ofor_volume_expansion_mult ?? 1.5;
  const useOf = s.ofor_use_orderflow_filter !== false;
  const volExpansionOk = !useOf || (volExp != null && volAvg != null && volAvg > 0 && volExp / volAvg >= volMult);
  const volUnknown = useOf && (volExp == null || volAvg == null);
  ...
  else if (phase === "pre") { modeStatus = "Waiting"; blocked = "Before 09:30 ET — range not started"; }
  ...
  if (connected) {
    if (phase === "building") msg = "Building Opening Range (09:30–10:00 ET).";
    else if (phase === "pre") msg = "Range starts at 09:30 AM ET.";
    else if (blocked && modeStatus === "Blocked") msg = `Blocked: ${blocked.toLowerCase()}.`;
    else if (breakout === "Buy pending close" && volUnknown) msg = "BUY breakout forming — awaiting order-flow confirmation.";
    else if (breakout === "Sell pending close" && volUnknown) msg = "SELL breakout forming — awaiting order-flow confirmation.";
    else if (breakout === "Buy pending close" && !volExpansionOk) msg = "BUY breakout forming — volume participation weak.";
    else if (breakout === "Sell pending close" && !volExpansionOk) msg = "SELL breakout forming — volume participation weak.";
    else if (breakout === "Buy pending close") msg = "BUY breakout + order flow confirmed — awaiting M5 close / retest.";
    else if (breakout === "Sell pending close") msg = "SELL breakout + order flow confirmed — awaiting M5 close / retest.";
    else msg = "Waiting for acceptance, not just breakout.";
  }

  // planned trade (illustrative based on current range + price proxy)
  let planned = null;
  const rr = s.ofor_risk_reward ?? 2;
  const slBuf = s.ofor_sl_buffer_points ?? 5;
  if (rangeOk && price != null && (breakout === "Buy pending close" || breakout === "Sell pending close")) {
    const isBuy = breakout === "Buy pending close";
    const retestRef = isBuy ? orHigh : orLow;
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
      <GlassCard className="p-0 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-white/5">
          <div className="flex items-center gap-2.5">
            <div className="w-10 h-10 rounded-xl bg-fuchsia-500/15 flex items-center justify-center">
              <Waves className="w-5 h-5 text-fuchsia-300" />
            </div>
            <div>
              <h3 className="font-heading text-sm font-black text-white tracking-wide leading-tight">{STRATEGY_NAME}</h3>
              <p className="text-[10px] text-white/40">{pair} · M5 · Opening Range 09:30–10:00 ET</p>
            </div>
          </div>
        ...
        <div className="mx-4 mt-3 rounded-xl px-3 py-2.5 flex items-center gap-2"
          style={{ background: modeStatus === "Blocked" ? "rgba(255,77,77,0.08)" : modeStatus === "Active" ? "rgba(0,255,157,0.08)" : "rgba(217,70,239,0.07)", border: `1px solid ${modeStatus === "Blocked" ? "rgba(255,77,77,0.25)" : modeStatus === "Active" ? "rgba(0,255,157,0.25)" : "rgba(217,70,239,0.25)"}` }}>
          {loading ? <Loader2 className="w-4 h-4 text-fuchsia-300 animate-spin" /> : modeStatus === "Blocked" ? <ShieldAlert className="w-4 h-4 text-[#ff4d4d]" /> : modeStatus === "Active" ? <CheckCircle2 className="w-4 h-4 text-[#00ff9d]" /> : <Activity className="w-4 h-4 text-fuchsia-300" />}
          <div className="flex-1 min-w-0">
            <p className="text-[10px] font-heading tracking-widest" style={{ color: modeStatus === "Blocked" ? "#ff9d9d" : modeStatus === "Active" ? "#9dffd4" : "#e8b4f8" }}>OPENING RANGE: {modeStatus.toUpperCase()}</p>
        ...
        <div className="grid grid-cols-2 gap-2 px-4 pt-2">
          <Chip label="Countdown to 10:00" value={cdToOpen != null ? fmtClock(cdToOpen) : "--"} tone={phase === "pre" || phase === "building" ? "warn" : "neutral"} />
        ...
        {/* Opening range levels + price */}
        <div className="grid grid-cols-3 gap-2 px-4 pt-2">
          <Chip label="Range High" value={fmt(orHigh, 1)} tone="up" />
          <Chip label="Range Mid" value={fmt(orMid, 1)} tone="neutral" />
          <Chip label="Range Low" value={fmt(orLow, 1)} tone="down" />
        </div>
        <div className="grid grid-cols-3 gap-2 px-4 pt-2">
        ...
        <div className="px-4 pt-3 space-y-1.5">
          ...
          <StatusRow icon={Waves} label="Order-Flow" value={volUnknown ? "No data" : volExpansionOk ? "Confirmed" : "Weak"} tone={volExpansionOk ? "up" : "warn"} />
        ...
        {/* Planned trade */}
        {planned && (
          <div className="mx-4 mt-3 rounded-xl px-3 py-3 space-y-1.5" style={{ background: "rgba(217,70,239,0.06)", border: "1px solid rgba(217,70,239,0.2)" }}>
            <div className="flex items-center justify-between">
              <p className="text-[10px] font-heading tracking-wider text-fuchsia-300">PLANNED TRADE</p>
        ...
        {blocked && modeStatus === "Blocked" ? (
          ...
          <div className="mx-4 mt-3 mb-4 flex items-center gap-2 rounded-xl px-3 py-2" style={{ background: "rgba(0,255,157,0.06)", border: "1px solid rgba(0,255,157,0.2)" }}>
            ...
            <p className="text-[10px] text-[#9dffd4]">Entry window open — waiting for acceptance, not just breakout.</p>
          </div>
        ) : (
          <div className="mx-4 mt-3 mb-4 flex items-center gap-2 rounded-xl px-3 py-2" style={{ background: "rgba(217,70,239,0.05)", border: "1px solid rgba(217,70,239,0.18)" }}>
            <Activity className="w-3.5 h-3.5 text-fuchsia-300 shrink-0" />
            <p className="text-[10px] text-white/60">{blocked ? `${blocked}.` : "Building range / waiting for order-flow confirmation."}</p>
          </div>
        )}
        ...
        <div className="px-4 pb-4 grid grid-cols-2 gap-x-4 gap-y-1 text-[10px]">
          {[
            ["Symbol", pair],
            ["Timeframe", "M5"],
            ["Lot Size", s.ofor_lot_size ?? "0.01"],
            ["Max Trades/Day", s.ofor_max_trades_per_day ?? 2],
            ["Risk Reward", `1:${s.ofor_risk_reward ?? 2}`],
            ["SL Buffer (pts)", s.ofor_sl_buffer_points ?? 5],
          ].map(([k, v]) => (
